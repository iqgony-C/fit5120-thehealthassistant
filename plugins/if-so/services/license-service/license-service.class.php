...
if( !class_exists( 'EDD_SL_Plugin_Updater' ) ) {
	...
	require_once plugin_dir_path( dirname( __FILE__ ) ) . 'EDD_SL_Plugin_Updater/EDD_SL_Plugin_Updater.php';
}
...
class LicenseService {
	...
	private $geo_array = array(9132, 9129, 8261, 6530);
	...
	public function edd_ifso_activate_license() {
		...
		if( isset( $_POST['edd_ifso_license_activate'] ) ) {
			...
			if(empty($message) && in_array($passed_license_id, $this->geo_array)){
                $geo_page_link = urlencode(admin_url('admin.php?page=' . EDD_IFSO_PLUGIN_GEO_PAGE));
                $message .= __("You have entered a Geolocation License. To activate a Geolocation License please go to",'if-so')." <a href='{$geo_page_link}'> If-So > Geolocation</a>.";
