    ...
    <table id="ifso-all-groups-table" class="widefat striped">
        ...
        if(isset($groups_list) && is_array($groups_list) && !empty($groups_list)){
            foreach ($groups_list as $group){
                ...
                $delme = admin_url('admin-ajax.php?action=ifso_groups_req&ifso_groups_action=remove_group&group_name=' . $group . '&_wpnonce=' . wp_create_nonce('ifso-groups-action-nonce'));
                echo "<tr>
                        <td> {$group}</td>
                        <td>{$occurences}</td>
