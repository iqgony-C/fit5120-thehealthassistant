...
<script>
	...
	jQuery.post(ajaxurl, data, function(response) { 
		...
		setTimeout(() => {
			...
			jQuery('#Rich_Web_VS_Icon_Opt_' + Option_ID).children('button').html('<i class="rich_web rich_web-check"></i> Deleted');
			setTimeout(() => {
			jQuery('#Rich_Web_VS_Opt_Content_'+Option_ID).removeAttr('id').attr('id','Rich_Web_VS_Opt_Content_'+responseData.slider_class);
			jQuery('#Rich_Web_VS_Opt_Content_'+responseData.slider_class).removeClass("Rich_Web_VS_Opt_Content_H");
			jQuery('#Rich_Web_VS_Opt_Content_'+responseData.slider_class).children('img').attr('alt',responseData.slider_Vid_type);
			jQuery('#Rich_Web_VS_Icon_Opt_' + Option_ID).removeAttr('id').attr('id','Rich_Web_VS_Icon_Opt_'+responseData.slider_class);
			jQuery('#Rich_Web_VS_Icon_Opt_'+responseData.slider_class).siblings('Rich_Web_VS_Name').text(responseData.slider_Vid_type);
			var name = String(responseData.slider_class);
			jQuery('#Rich_Web_VS_Icon_Opt_'+responseData.slider_class).html('<button class="rw_vs_but_demo" ><i class="Rich_Web_VS_Demo rich_web rich_web-eye"></i></button>  <button class="rw_vs_but_install" ><i class="Rich_Web_VS_Install rich_web rich_web-cloud-download"></i>Insert</button> ')
			jQuery('#Rich_Web_VS_Icon_Opt_'+responseData.slider_class).children('.rw_vs_but_demo').attr('onclick','Rich_Web_VS_Demo('+`"${name}"`+')');
			jQuery('#Rich_Web_VS_Icon_Opt_'+responseData.slider_class).children('.rw_vs_but_install').attr('onclick','Rich_Web_VS_Insert('+`"${name}"`+')')
			}, 600);
		}
	 });				
	}
	...
	function Rich_Web_VS_Insert(RW_VS_Type){
		...
		if(!navigator.onLine) {
			rw_vs_opt_icon.html('<button class="rw_vs_icon_spinner" style="background-color: #ff7878;background-image: linear-gradient(315deg, #ff7878 0%, #ff0000 74%);" ><i class="rich_web rich_web-warning  " style="margin-right:10px;animation: scaledAnim 0.5s alternate infinite ease-in; color:#ffffff;" ></i>No Internet</button>');
   		}else{
		var ajaxurl = object.ajaxurl;
		var data = {
