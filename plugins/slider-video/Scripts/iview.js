(function ($, window, undefined) {
	var iView = function (el, options) {
			...
			iv.sliderContent.bind('iView:goSlide', function (event, slide) {
				...
				iv.slider.css('background-size', ''+jQuery("#iview").width()+'px '+jQuery("#iview").height()+'px');
			...
			$('.iview-video', iv.slider).click(function(e){
				...
				jQuery('.iview-controlNav').css('display','none');
			});
			$('.iview-video-close', iv.slider).click(function(){
				...
				video.animate({ top: '-' + iv.defs.height + 'px' }, 1000, 'easeOutBounce', function(){
					...
					jQuery('.iview-controlNav').css('display','block');
				});
	...
	iView.prototype = {
		...
		addStrips: function (vertical, opts) {
			...
			for (var i = 0; i < opts.strips; i++) {
				...
				var strip = $('<div class="iview-strip"></div>').css({
					...
					backgroundSize: ''+jQuery("#iview").width()+'px '+jQuery("#iview").height()+'px',
					opacity: 0
				});
		...
		addBlocks: function () {
			var iv = this,
				blockWidth = Math.round(iv.slider.width() / iv.options.blockCols)+1,

				blockHeight = Math.round(iv.slider.height() / iv.options.blockRows)+1;
