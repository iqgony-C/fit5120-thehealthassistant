				...
				<tbody id="update_div" name="update_div">
					...
					if($all_category) {
						...
						<tr id="record-<?php echo esc_attr($key); ?>">
							...
							<td id="cat_name" name="cat_name"><?php echo ucwords(esc_html($value)); ?></td>
							<td>&nbsp;
								<?php if($key != 0 ) { ?><i class="fa fa-pencil-square cat_icon" id="update_category" name="update_category"  onclick="return DoAction('edit', '<?php echo esc_attr($key); ?>');"></i><?php } ?>&nbsp;&nbsp;&nbsp;
								<?php if($key != 0 ) { ?><i class="fa fa-trash cat_icon" id="delete_category" name="delete_category" onclick="return pfg_delete_filter('<?php echo esc_attr($key); ?>');"></i><?php } ?>
							</td>
